...
═══════════════════════════════════════════════════════════
   ...
   4. ESPN FETCH LAYER (all sports) line ~320
      - espn(), getSeasonYear(), fetchTeamForm(),
        enrichFormWithPlayerStats(), fetchTeamStats()
   5. NBA-SPECIFIC FETCHERS ...... line ~484
      - fetchPlayerPropData(), fetchInjuries(), fetchH2H()
   6. PLAYER MODAL ............... line ~670
   7. NBA-ONLY: RESEARCH TAB ..... line ~864
   8. ROSTER FETCH ............... line ~1118
   9. RENDER FUNCTIONS ........... line ~1325
      - renderOverview(), renderRoster(), renderForm(),
        renderH2H(), renderProps()
  10. AI PLAYS (Claude API) ...... line ~2060

   SPORT-SPECIFIC CODE (loaded via <script> before app.js):
      sports/nba/config.js — Lineups, H2H stats, Edge Finder
      sports/mlb/config.js — Pitching Edge
...
async function startAnalysis(gameInfo) {
  ...
  try {
    ...
    const mlbEdgesTab = document.getElementById('tabMlbEdges');
    if (gameInfo.sportKey === 'mlb') {
      pitchingTab.style.display = '';
      lowHrTab.style.display = '';
      mlbEdgesTab.style.display = '';
      ...
      document.getElementById('mlbEdgesContent').innerHTML = '<div class="pe-empty">Analyzing batter edges...</div>';
      ...
      buildMlbEdgeFinderData(S.gameData).then(mlbEdgeData => {
        S.gameData.mlbEdgeData = mlbEdgeData;
        renderMlbEdgeFinder(mlbEdgeData, S.gameData);
      }).catch(e => {
        console.error('MLB Edge Finder error:', e);
        document.getElementById('mlbEdgesContent').innerHTML = '<div class="pe-empty">Edge analysis failed to load.</div>';
      });
    } else {
      pitchingTab.style.display = 'none';
      lowHrTab.style.display = 'none';
      mlbEdgesTab.style.display = 'none';
    }
...
/* ── NBA Lineups, H2H Stats, Edge Finder → sports/nba/config.js ── */
/* ── MLB Pitching Edge → sports/mlb/config.js ── */
